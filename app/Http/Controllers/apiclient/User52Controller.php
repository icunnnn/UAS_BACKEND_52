...
class User52Controller extends Controller
{
    public function dashboardPage52()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user52")->getBody()->getContents();
    ...
    public function profilePage52()
    {
        ...
        $getUser = $client->request('GET', "{$API_URL}/user52/{$user->id}")->getBody()->getContents();
        $getAgama = $client->request('GET', "{$API_URL}/agama52")->getBody()->getContents();
    ...
    public function detailPage52(Request $request, $id)
    {
        $API_URL = env('API_URL', "http://localhost/BackEnd/UAS-Backend/public/api");

        $client = new Client();
        $getUser = $client->request('GET', "{$API_URL}/user52/{$id}")->getBody()->getContents();
        $user = json_decode($getUser, true)['data'];
    ...
    public function putUserDetail52(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'name' => 'required',
            'email' => 'required|email|unique:users52,email,' . $user->id,
            'alamat' => 'required',
        ...
        $putUser = $client->request('PUT', "{$API_URL}/user52/{$user->id}", [
            'json' => [
                'name' => $request->name,
                'email' => $request->email,
    ...
    public function putUserAgama52(Request $request, $id)
    {
        ...
        $putUserAgama = $client->request('PUT', "{$API_URL}/user52/{$id}/agama", [
            'json' => [
    ...
    public function putUserStatus52(Request $request, $id)
    {
        ...
        $putUserStatus = $client->request('PUT', "{$API_URL}/user52/{$id}/status", [
            'json' => [
    ...
    public function putUserPassword52(Request $request)
    {
        ...
        $putUserPassword = $client->request('PUT', "{$API_URL}/user52/{$user->id}/password", [
            'json' => [
    ...
    public function putUserPhoto52(Request $request)
    {
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user52/{$id}/photo", [
            'headers' => [
    ...
    public function putUserPhotoKTP52(Request $request)
    {
        $API_URL = env('API_URL', "http://localhost/BackEnd/UAS-Backend/public/api");
        ...
        $putUserPhoto = $client->request('PUT', "{$API_URL}/user52/{$id}/photoKTP", [
            'headers' => [
                'Accept' => 'application/json',
            ],
            'json' => [
    ...
    public function deleteUser52($id)
    {
        $API_URL = env('API_URL', "http://localhost/BackEnd/UAS-Backend/public/api");

        $client = new Client();

        $deleteUser = $client->request('DELETE', "{$API_URL}/user52/{$id}", [
            'headers' => [
                'Accept' => 'application/json',
            ],
        ])->getBody()->getContents();
